...
export class FileSelectionUi {
  ...
  private readonly getTranslation: (key: string) => string;
  ...
  private constructor(
    ...
    this.fileScanInput = BaseUiElementFactory.createElement<HTMLInputElement>(
      "input",
      ...
      if (dataTransfer) {
        ...
        for (let i = 0; i < files.length; ++i) {
          ...
          dragAndDropMessage.innerText =
            this.getTranslation("dragAndDropMessage");
          break;
        }

        // None of the files were images.
        if (!isAnyFileImage) {
          dragAndDropMessage.innerText = this.getTranslation(
  ...
  private createDragAndDropMessage(): HTMLDivElement {
    ...
    dragAndDropMessage.innerText = this.getTranslation("dragAndDropMessage");
  ...
  private setImageNameToButton(imageFileName: string) {
    ...
    let newText =
      this.getTranslation("fileSelectionChooseAnother") + " - " + imageFileName;
    this.fileSelectionButton.innerText = newText;
  }

  private setInitialValueToButton() {
    let initialText =
      this.getTranslation("fileSelectionChooseImage") +
      " - " +
      this.getTranslation("fileSelectionNoImageSelected");
