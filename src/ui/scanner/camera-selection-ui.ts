...
export class CameraSelectionUi {
  ...
  private constructor(
    ...
    this.selectElement = BaseUiElementFactory.createElement<HTMLSelectElement>(
      "select",
  ...
  private render(parentElement: HTMLElement) {
    ...
    if (numCameras === 1) {
      ...
      const selectCameraString = this.getTranslation("selectCamera");
    ...
    for (const camera of this.cameras) {
      ...
      if (!name || name === "") {
        name = [
          this.getTranslation("anonymousCameraPrefix"),
          anonymousCameraId++,
        ].join(" ");
      }
  ...
  public static create(
    ...
    let cameraSelectUi = new CameraSelectionUi(cameras, getTranslation);
