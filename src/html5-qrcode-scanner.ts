...
export class Html5QrcodeScanner {
  ...
  private translations: Html5QrcodeTranslations;
   ...
   * @param translations - Optional, translations for the strings used in the UI.
   */
  public constructor(
    elementId: string,
    config: Html5QrcodeScannerConfig | undefined,
    verbose: boolean | undefined,
    translations?: Html5QrcodeTranslations
  ) {
    ...
    this.qrCodeSuccessCallback = (
      ...
      if (qrCodeSuccessCallback) {
        ...
        this.setHeaderMessage(
          this.getTranslation("lastMatch", decodedText),
  ...
  private createCameraListUi(
    ...
    $this.setHeaderMessage(this.getTranslation("cameraPermissionRequesting"));
    ...
    Html5Qrcode.getCameras()
      .then((cameras) => {
        ...
        if (cameras && cameras.length > 0) {
          ...
          $this.setHeaderMessage(
            this.getTranslation("noCameraFound"),
      ...
      .catch((error) => {
        $this.persistedDataManager.setHasPermission(/* hasPermission */ false);
  ...
  private createPermissionButton(
    ...
    requestPermissionButton.innerText = this.getTranslation(
  ...
  private createSectionControlPanel() {
    ...
    scpCameraScanRegion.style.display = ScanTypeSelector.isCameraScanType(
  ...
  private renderFileScanUi(parent: HTMLDivElement) {
    ...
    let onFileSelected: OnFileSelected = (file: File) => {
      ...
      $this.setHeaderMessage(this.getTranslation("loadingImage"));
      $this.html5Qrcode
        ...
        .catch((error) => {
          ...
          $this.qrCodeErrorCallback!(
            error,
            Html5QrcodeErrorFactory.createFrom(error)
          );
        });
    };
    ...
    this.fileSelectionUi = FileSelectionUi.create(
      ...
      showOnRender,
      onFileSelected,
      (key: string) => this.getTranslation(key as keyof Html5QrcodeTranslations)
    );
  }
  ...
  private renderCameraSelection(cameras: Array<CameraDevice>) {
    ...
    cameraActionStartButton.innerText = this.getTranslation(
    ...
    const cameraActionStopButton =
      BaseUiElementFactory.createElement<HTMLButtonElement>(
        "button",
        PublicUiElementIdAndClasses.CAMERA_STOP_BUTTON_ID
      );
    cameraActionStopButton.innerText = this.getTranslation(
    ...
    const createAndShowTorchButtonIfSupported = (
      ...
      if (!torchButton) {
        torchButton = TorchButton.create(
          ...
          (key: string) =>
            this.getTranslation(key as keyof Html5QrcodeTranslations)
        );
      } else {
  ...
  private createSectionSwap() {
    const $this = this;
    const TEXT_IF_CAMERA_SCAN_SELECTED = this.getTranslation(
      "textIfCameraScanSelected"
    );
    const TEXT_IF_FILE_SCAN_SELECTED = this.getTranslation(
    ...
    switchScanTypeLink.innerText = ScanTypeSelector.isCameraScanType(
      this.currentScanType
    )
      ? TEXT_IF_CAMERA_SCAN_SELECTED
      : TEXT_IF_FILE_SCAN_SELECTED;
  ...
  private setHeaderMessage(
    messageText: string,
    scannerStatus?: Html5QrcodeScannerStatus
  ) {
    if (!scannerStatus) {
  ...
  private showHideScanTypeSwapLink(shouldDisplay?: boolean) {
    if (this.scanTypeSelector.hasMoreThanOneScanType()) {
      ...
      this.getDashboardSectionSwapLink().style.display = shouldDisplay
        ? "inline-block"
        : "none";
    }
  }
  ...
  private insertCameraScanImageToScanRegion() {
    ...
    this.cameraScanImage.alt = this.getTranslation("cameraScanAltText");
  }
  ...
  private insertFileScanImageToScanRegion() {
    ...
    this.fileScanImage.alt = this.getTranslation("fileScanAltText");
  }
  ...
  private getTranslation<K extends keyof Html5QrcodeTranslations>(
    key: K,
    ...args: [any?]
  ): string {
    ...
    // fallback
    const fallback =
      Html5QrcodeScannerStrings[key as keyof typeof Html5QrcodeScannerStrings];
